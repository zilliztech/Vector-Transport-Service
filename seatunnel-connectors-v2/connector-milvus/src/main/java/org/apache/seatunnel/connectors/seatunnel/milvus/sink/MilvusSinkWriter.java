...
public class MilvusSinkWriter implements SinkWriter<SeaTunnelRow, MilvusCommitInfo, MilvusSinkState> {
    ...
    public void write(SeaTunnelRow element) {
        ...
        if (batchWriter.needFlush()) {
            try {
                log.info("Starting to put {} records to Milvus.", config.get(BATCH_SIZE));
                // Flush the batch writer
                batchWriter.flush();
                // Get the number of records completed
                long recordsWritten = batchWriter.getRecordsWritten();
                log.info("Successfully put {} records to Milvus. Total records written: {}", config.get(BATCH_SIZE), recordsWritten);
            } catch (Exception e) {
